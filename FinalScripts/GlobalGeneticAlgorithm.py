...
class GlobalGeneticAlgorithmOptimization:
    ...
    def solve(self, alphas, t):
        ...
        R0 = self.sc_params[0]
        m = self.sc_params[1]
        delta = self.sc_params[2]
        S = self.sc_params[3]
        Aref = self.sc_params[4]
        L = np.zeros(len(t))
        D = np.zeros(len(t))
        CL = np.zeros(len(t))
        CD = np.zeros(len(t))
        ...
        for i in range(1, len(t) - 1):
            ...
            args = (g, m, gamma, R0, delta, alphas[i], S)
            ...
            q[i] = getStagHeatFlux(h[i], M[i], gamma, Pr, R0)  # Fay-Riddle stagnation point heat flux approximation [W/m^2]
    ...
    def check_feasibility(self, params):
        ...
        h = opt[7]
        """
        dalphas = np.zeros(len(alphas))
        for i in range(0, len(alphas)-1):
            dalphas[i] = abs(alphas[i+1] - alphas[i])
        """
        #dalpha = max(dalphas)
